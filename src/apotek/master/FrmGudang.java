 ...
 * @author root
 */
public class FrmGudang extends javax.swing.JInternalFrame {
    ...
    public FrmGudang() {
        initComponents();
        tblGudang.getSelectionModel().addListSelectionListener(new SelectionListener(tblGudang));
    }
    ...
    public void initJDBC() {
        String sQry = "select kode_gudang, coalesce(deskripsi,'') as deskripsi "
        ...
        try {
            ...
            while (rs.next()) {
                modelJenis.addRow(new Object[]{rs.getString("kode_gudang"),
                    rs.getString("deskripsi"),});
            }

            SelectionListener listener = new SelectionListener(tblGudang);
            tblGudang.getSelectionModel().addListSelectionListener(listener);
            tblGudang.getColumnModel().getSelectionModel().addListSelectionListener(listener);
            tblGudang.setRequestFocusEnabled(true);

            if (modelJenis.getRowCount() > 0) {
                tblGudang.setRowSelectionInterval(0, 0);
            }
    ...
    private void TableLook() {
        tblGudang.getColumnModel().getColumn(0).setMaxWidth(70);     //kode
        tblGudang.getColumnModel().getColumn(0).setPreferredWidth(70);

        tblGudang.setRowHeight(20);
        for (int i = 0; i < tblGudang.getColumnCount(); i++) {
            tblGudang.getColumnModel().getColumn(i).setCellRenderer(new MyRowRenderer());
        }

        if (modelJenis.getRowCount() > 0) {
            tblGudang.changeSelection(0, 0, false, false);
        }
    }
...
//                    onOpen(cmbFilter.getSelectedItem().toString(),true);
                ...
                case KeyEvent.VK_F12: {  //Delete
                    if (!getBEdit() && tblGudang.getRowCount() > 0) {
                        udfUpdateData();
                    }
...
//            if (lst.isVisible()) lst.setVisible(false);
    ...
    public void setUpBtn() {
        if (getBEdit()) {  //proses edit     
            ...
            tblGudang.setEnabled(false);
            txtNama.setEditable(true);

            tblGudang.requestFocus();
...
//        sBean.setNama(txtNama.getText());
        ...
        if (getBEdit()) {
            try {
                ...
                if (getBNew()) {        //Add
                    ...
                    ResultSet rs = st.executeQuery("select fn_save_gudang('', '" + txtNama.getText() + "')");
                    ...
                    if (rs.next()) {
                        ...
                        modelJenis.addRow(new Object[]{txtKode.getText(),
                            txtNama.getText(),});
                        setBEdit(false);
                        tblGudang.setRowSelectionInterval(modelJenis.getRowCount() - 1, modelJenis.getRowCount() - 1);
                        setBEdit(false);
                        setBNew(false);
                        setUpBtn();
                    } else {
                        pesanError("Insert master gudang gagal. Silakan coba sekali lagi!");
                    ...
                    Statement st = conn.createStatement();
                    ResultSet rs = st.executeQuery("select fn_save_gudang('" + txtKode.getText() + "', '" + txtNama.getText() + "')");
                    ...
                    if (!rs.next()) {
                        ...
                        tblGudang.setValueAt(txtNama.getText(), tblGudang.getSelectedRow(), 1);
                        System.out.println(hsl);
                        setBEdit(false);
                        setBNew(false);
                        setUpBtn();
            ...
            try {
                String s = tblGudang.getValueAt(tblGudang.getSelectedRow(), 1).toString();
                if (JOptionPane.showConfirmDialog(null, "Anda Yakin Untuk mengapus '" + s + "' ?", "SHS go Open Sources", JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
                    int iPosDel = tblGudang.getSelectedRow();

                    Statement st = conn.createStatement();
                    hsl = st.executeUpdate("delete from gudang where kode_gudang='" + txtKode.getText() + "'");

                    conn.commit();
                    modelJenis.removeRow(iPosDel);
                    if (iPosDel < modelJenis.getRowCount() && modelJenis.getRowCount() > 0) {
                        tblGudang.setRowSelectionInterval(iPosDel, iPosDel);
                        tblGudang.requestFocus();
                    }
                }
            } catch (SQLException se) {
    ...
    private void udfCancel() {
        if (getBEdit()) {
            ...
            setBNew(false);
            if (tblGudang.getRowCount() > 0) {
                int rowPos = tblGudang.getSelectedRow();
                txtKode.setText(tblGudang.getValueAt(rowPos, 0).toString());
                txtNama.setText(tblGudang.getValueAt(rowPos, 1).toString());
            }
            setUpBtn();
            tblGudang.setRequestFocusEnabled(true);
    ...
    private void udfFilter() {
        modelJenis = (DefaultTableModel) tblGudang.getModel();
        int i = 0;
        try {
            String sQry = "select kode_gudang, coalesce(deskripsi,'') as nama_gudang "
                    + "from gudang "
                    + "where kode_gudang||coalesce(deskripsi,'') ilike '%" + txtSearch.getText() + "%' "
                    + "order by 1";
            ...
            if (modelJenis.getRowCount() > 0) {
                tblGudang.setRowSelectionInterval(0, 0);
                fn.setAutoResizeColWidth(tblGudang);
            }
            rs.close();
        ...
        if (i > 0) {
            tblGudang.requestFocusInWindow();
            tblGudang.setRowSelectionInterval(0, 0);
        }
    }
    ...
    private void initComponents() {
        ...
        tblGudang.setAutoCreateRowSorter(true);
        ...
        tblGudang.getTableHeader().setReorderingAllowed(false);
    ...
    private void formInternalFrameOpened(javax.swing.event.InternalFrameEvent evt) {//GEN-FIRST:event_formInternalFrameOpened
        ...
        tblGudang.addKeyListener(new MyKeyListener());
    ...
    private void btnDeleteActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnDeleteActionPerformed
        if (tblGudang.getRowCount() > 0 || getBEdit()) {
            udfUpdateData();
        }
