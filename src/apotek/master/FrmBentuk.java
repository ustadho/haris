 ...
 * @author root
 */
public class FrmBentuk extends javax.swing.JInternalFrame {
        ...
        if (getBEdit()) {
            try {
                ...
                if (getBNew()) {        //Add
                    ...
                    ResultSet rs = st.executeQuery("select fn_save_item_bentuk('', '" + txtNama.getText() + "')");
                    ...
                    Statement st = conn.createStatement();
                    ResultSet rs = st.executeQuery("select fn_save_item_bentuk('" + txtKode.getText() + "', '" + txtNama.getText() + "')");
...
//                    conn.commit();
    ...
    private void udfFilter() {
        ...
        int i = 0;
        try {
            String sQry = "select bentuk_id, coalesce(bentuk_name,'') as bentuk_name "
                    + "from item_bentuk "
                    + "where bentuk_id||coalesce(bentuk_name,'') ilike '%" + txtSearch.getText() + "%' "
                    + "order by 1";
            ...
            while (rs.next()) {
                modelJenis.addRow(new Object[]{rs.getString("bentuk_id"),
